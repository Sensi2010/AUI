...
local _ -- This is used in a couple of places
local g_isInit = false
local g_windows = nil
local g_defaultAnchors = {} -- This is not used
local g_isPreviewShowing = false
local g_currentInputMode = 1
local g_inputModeList = { -- This is not used
    ["keyboard"] = 1,
    ["gamepad"] = 2,
}
...
--  if _button == 1 then
    if _button == MOUSE_BUTTON_INDEX_LEFT then -- the global for button 1
...
local function OnFrameMouseUp(_button, _ctrl, _alt, _shift, _frame)
    ...
    -- This would only work if setting to GuiRoot by default. Otherwise, need to fish out the true parent to send.
...
function AUI.FrameMover.UpdateAll()
    ...
    for windowName, inputData in pairs(g_windows) do
        local anchorTo = GuiRoot -- Don't want those withought a defined parent to acnchor to the preiouse parent.
        ...
        if windowData then
            ...
            if not windowData.mainControl then
                ...
                -- First param of handlers is the control that ownes the handler. Added _ to the shift as it was missing.
